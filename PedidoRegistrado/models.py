...
from ComponentesDePedido.models import DetalleVersiones, Menu, Promocion, \
    TipoProducto, Version, Clasificacion, TipoIngrediente, Ingrediente
...
class Servicio(models.Model):
    ...
    def Vista_Previa(self):
        texto = '<a href="/media/%s"><img src="/media/%s" width=50px'
        texto += 'heigth=50px/></a>'
        return texto % (self.imagen, self.imagen)
...
class DomicilioSearch(models.Model):
    ...
    Atributos: direccion,numero_direccion,piso,depto,
    ...
    barrio = ChainedForeignKey(Barrio, chained_field="localidad",
                        chained_model_field="localidad", auto_choose=True)
...
class Pedido(models.Model):
    """Clase Pedido
    Atributos:cliente, fechaPedido, hora_entrega, estado, servicio,
    tipologia_vivienda, precio_envio, precio_total"""
...
class VersionProducto(models.Model):
    ...
    imagenProducto = models.ImageField(upload_to='imagenes/DetalleVersiones',
                                       verbose_name='Imágen')
    precio = models.DecimalField(max_digits=5, decimal_places=2,
                                 verbose_name="Precio por clasificación($)")
    tiempoPreparacion = models.IntegerField(max_length=10,
                        verbose_name="Tiempo Preparacion estimado(minutos)")
...
class SeccionProducto(models.Model):
    ...
    excluyente = models.BooleanField(
        verbose_name="Clasificaciones Excluyentes")
    cantidad_exclusiones = models.IntegerField(
        verbose_name="Cantidad Exclusiones")
...
class IngredientesSeccion(models.Model):
    ...
    def __unicode__(self):
        return u'%s - %s - %s' % (self.seccion.producto.slogan,
                                  self.seccion.nombre, self.ingrediente.nombre)
...
class IngredienteClasificacion(models.Model):
    ...
    def __unicode__(self):
        texto = self.ingrediente.ingrediente.nombre
        texto += self.ingrediente.seccion.nombre + self.clasificacion.nombre
        return texto
...
class DetalleProductoArmado(models.Model):
    ...
    def __unicode__(self):
        return u'%s - %s' % (self.producto.producto.slogan,
                             self.ingrediente.seccion)
